...
const Navigation: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Mobile Navigation - 5 Button Layout */}
        ...
        <div className="flex justify-between px-1 py-2">
          {navigationItems.map(({ id, label }) => (
            <button
              key={id}
              onClick={() => scrollToSection(id)}
              className={`relative px-1.5 py-2 rounded-xl text-xs font-medium transition-all duration-300 min-h-[44px] min-w-[44px] flex items-center justify-center focus-visible:ring-enhanced ${
                activeSection === id
          ...
          <button
            onClick={toggleTheme}
            className="relative px-1.5 py-2 rounded-xl text-xs font-medium transition-all duration-300 min-h-[44px] min-w-[44px] flex items-center justify-center focus-visible:ring-enhanced bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 hover:bg-orange-200 dark:hover:bg-orange-900/50"
            aria-label="Toggle theme"
          >
